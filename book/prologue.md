...
In the current quantitative implementation this framework uses two external continuous inputs: the pixel area $P \approx 1.63$, the geometric size of a single computational element on the holographic screen in Planck units, and the total screen capacity inferred from the observed cosmological constant. Everything else — Newton's constant, the gauge structure, and the effective spacetime branch — follows from the axioms or from stated structural conditions. The Standard Model gauge structure is derived under the MAR admissibility package. The downstream particle ledger is explicitly tiered rather than uniformly closed: the photon, gluons, and graviton are structural massless carriers; the electroweak calibration lane is closed on a target-free source-only theorem that emits the $W$ and $Z$ rows; the Higgs/top critical stage emits the Higgs boson and top quark as secondary quantitative rows; the quark branch emits continuation-level rows for the up, down, strange, charm, and bottom quarks while the physical CKM branch still waits on the finite same-label left-handed orbit and the provider/evaluator that exposes the first non-empty orbit elements; the charged-lepton lane fixes the centered hierarchy but still waits on the refinement-stable uncentered trace lift beneath the determinant-line route to emit the absolute scale; flavor-labeled neutrinos have the weighted-cycle PMNS/hierarchy branch and closed normalizer in hand but still wait on the residual bridge scalar $B_\nu$ and one positive normalization scalar; and hadrons remain execution-bound because promotable rows still require real nonperturbative production computation and systematics rather than another symbolic theorem.
