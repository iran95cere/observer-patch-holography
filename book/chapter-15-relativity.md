...
Consider a cap \(C\) on the sphere \(S^2\). In the regime where the screen admits a smooth geometric description, the cap's natural thermal time stops being just an abstract algebraic flow and becomes an actual geometric motion on the sphere. That is the crucial step. Relativity needs more than a formal clock. It needs a clock whose flow can be read as boosts and time translations.

In compact form, the statement is

$$\sigma_t^{\omega_C} = \alpha_{\lambda_C(2\pi t)}$$

The equation says that the modular flow \(\sigma_t^{\omega_C}\) generated by the state on the cap agrees with a conformal motion \(\alpha_{\lambda_C(2\pi t)}\) of the sphere. In a particularly simple realization, one can write the generator itself as
