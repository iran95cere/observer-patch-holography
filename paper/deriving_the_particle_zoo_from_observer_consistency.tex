...
The Stage-5 summary is this: quarks close on the selected public quark frame class \(f_P\); charged leptons have an exact same-carrier readback theorem, a conditional determinant-line lift on theorem-grade physical charged data, and an algebraic mass readout from theorem-grade \(A_{\mathrm{ch}}(P)\), with one open source theorem from \(P\) to physical charged data; neutrinos carry an emitted weighted-cycle bridge-rigid absolute family above the scale-free branch. None of those later-family statements belongs on the recovered-core surface fixed earlier by the gauge and relativity chain.
